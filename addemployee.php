  ...
  <body>
	...
	<?php
	 $update_only_str=file_get_contents('main.json');
		...
		echo "</tr></thead><tr>";
		foreach($main_decode["user-th-placeholder"] as $key=>$value){
		echo "<td><input type='text' placeholder='$value' class='form-control' name='$key'></td>";
		}
		...
		?>
			...
			<tr><td><input type="submit" class="btn btn-success" value="Submit"></td><td><a href='employees.php'><input type='button'value='Cancel' class="btn btn-warning"></a></td>
	</tr>
		</table>
		</form>
	...
	//$main_decode["schedule"]
	$name=$_POST['name'];
	foreach($main_decode["w-days"] as $key=>$day)
	{
		$schedule["$key"]="OFF";
		
	}
	$main_decode['schedule']["$name"]=$schedule;
	
	$main_decode['users']["$name"]=$data;
